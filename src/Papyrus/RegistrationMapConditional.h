...
namespace SKSE
{
	...
	namespace C3
	{
		...
		public:
			...
			// NOTE: Conditional registration map
			...
			template <>
			class RegistrationMapConditional<void> : public RegistrationMapBase
			{
			private:
				using super = RegistrationMapBase;
				using PassFilterFunc = std::function<bool(const Filter&)>;
			...
			public:
				...
				inline void SendEvent(PassFilterFunc a_filter)
				{
					RE::BSFixedString eventName(this->_eventName);
				...
				inline void QueueEvent(PassFilterFunc a_filter)
				{
					auto task = GetTaskInterface();
					assert(task);
	...
	using RegistrationMapConditional = typename C3::EventFilter<Filter>::template RegistrationMapConditional<void, Args...>;
